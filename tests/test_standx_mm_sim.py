...
Usage:
    python test_exchanges/test_standx_mm_sim.py
"""
...
async def main():
    ...
    try:
        ...
        # LIVE 모드: 종료 시 주문 취소 옵션
        if is_live and order_mgr.get_open_orders():
            console.print("[yellow]Open orders detected. Cancel all? (y/n): [/yellow]", end="")
            try:
                cancel_choice = input()
